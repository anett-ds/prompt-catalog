...
function Field({ fieldDef, value, onChange, th }: { fieldDef: FieldDef; value: string; onChange: (v: string) => void; th: Theme }) {
  ...
  const focus = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => (e.target.style.borderColor = '#7c3aed');
  const blur = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => (e.target.style.borderColor = th.inputBorder);
  return (
...
export default function App({ onLogout }: { session: Session; onLogout: () => void }) {
